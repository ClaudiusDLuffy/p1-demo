-- Server-side Work Orders table sorting and per-column filtering. This is a
-- dedicated read RPC so existing dashboards/history pages keep their proven
-- cursor contract while the interactive table can sort the complete result
-- set rather than only the ten rows currently in the browser.
