-- Run after migration 0107. This is read-only. Every boolean should be true,
-- every issue count should be zero, and all_checks_pass should be true.

with work_order_access as (
  select
    procedure.prosecdef,
  ...
  select
    ...
    ) as active_canonical_link_count,
    count(*) filter (
      where technician.profile_id = rush.profile_id
        and technician.contractor_id is distinct from context.contractor_id
        and technician.is_active = true
  ...
  select
    ...
    link.active_canonical_link_count = 1
    ...
    aliases.active_unlinked_or_wrong_profile_alias_count = 0
      as all_rush_alias_duplicates_removed,
...
select
  ...
  contractor_company_and_assignment_wall_preserved
    ...
    and all_rush_alias_duplicates_removed
