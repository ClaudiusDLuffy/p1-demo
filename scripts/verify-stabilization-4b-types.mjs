...
// This is a batch-specific ratchet against the independently verified pre-4B
// snapshot, not a replacement for the repository's ordinary TypeScript check.
const expectedChecksums = "0d1fa14dcfd678e972cfb3f697d717766181371dbf288ac17cec9a841a8773fe";
const repository = process.cwd();
const snapshotSetting = process.env.P1_4B_SNAPSHOT;
if (!snapshotSetting) throw new Error("P1_4B_SNAPSHOT must identify the verified pre-4B recovery snapshot");
...
assert.equal(digest(checksumsText), expectedChecksums, "Pre-4B checksum manifest does not match the verified receipt");
...
assert.ok(roots.length, "No Phase 4B roots were discovered");
...
function baselineSource(path) {
  ...
  if (entry) {
    ...
    assert.equal(digest(source), entry.sha256, `Reconstructed pre-4B source hash mismatch: ${path}`);
...
for (const path of roots) {
  ...
  for (const directive of current) {
    ...
    assert.ok(index !== -1, `New TypeScript suppression in Phase 4B root: ${path}`);
...
for (const path of legacyRoots) {
  assert.equal(digest(baselineProgram.getSourceFile(resolve(path))?.text ?? ""), digest(baselineSource(path)), `Compiler did not use pre-4B source: ${path}`);
}
