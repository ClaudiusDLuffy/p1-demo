...
// This is a batch-specific ratchet against the independently verified pre-5A
// snapshot, not a replacement for the repository's ordinary TypeScript check.
const expectedChecksums = "97e2ef192f00ff96582a53be9752e1a8045ecfae02bea1637ae192de0aa57006";
const repository = process.cwd();
const snapshotSetting = process.env.P1_5A_SNAPSHOT;
if (!snapshotSetting) throw new Error("P1_5A_SNAPSHOT must identify the verified pre-5A recovery snapshot");
...
assert.equal(digest(checksumsText), expectedChecksums, "Pre-5A checksum manifest does not match the verified receipt");
...
assert.ok(roots.length, "No Phase 5A roots were discovered");
...
function baselineSource(path) {
  ...
  if (entry) {
    ...
    assert.equal(digest(source), entry.sha256, `Reconstructed pre-5A source hash mismatch: ${path}`);
...
for (const path of roots) {
  ...
  for (const directive of current) {
    ...
    assert.ok(index !== -1, `New TypeScript suppression in Phase 5A root: ${path}`);
...
for (const path of legacyRoots) {
  assert.equal(digest(baselineProgram.getSourceFile(resolve(path))?.text ?? ""), digest(baselineSource(path)), `Compiler did not use pre-5A source: ${path}`);
}
