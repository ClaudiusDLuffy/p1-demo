...
const snapshot = process.env.P1_4B_SNAPSHOT;
assert(snapshot && path.isAbsolute(snapshot), "P1_4B_SNAPSHOT must identify the verified local recovery directory");
const digest = (bytes) => createHash("sha256").update(bytes).digest("hex");
const read = (relative) => readFileSync(path.join(snapshot, relative));
const manifestBytes = read("SHA256SUMS.json");
assert.equal(digest(manifestBytes), "0d1fa14dcfd678e972cfb3f697d717766181371dbf288ac17cec9a841a8773fe", "Recovery manifest changed");
