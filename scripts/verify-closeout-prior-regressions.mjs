...
try {
  ...
  await verifyPriorPaginationSchema(check, 146);
  const diagnostics = await createDatabase();
  try {
    await applyThrough(diagnostics, 146);
  ...
  try {
    ...
    await applyThrough(sla, 146, 141);
  ...
  console.log(JSON.stringify({ finalMigration: 146, priorPhaseChecks: checks, failed: 0,
