...
try {
  ...
  await verifyPriorPaginationSchema(check, 147);
  const diagnostics = await createDatabase();
  try {
    await applyThrough(diagnostics, 147);
  ...
  try {
    ...
    await applyThrough(sla, 147, 142);
  ...
  console.log(JSON.stringify({ finalMigration: 147, priorPhaseChecks: checks, failed: 0,
