...
// Accepted operations are created against actual 0127, then replayed after the
// 0128 schema expansion. Nullable columns must not invalidate old snapshots.
export async function preparePreAssignmentFinancialReplay({ db,as,actors }) {
  ...
  return { financial,verify:async check=>{
    await check('accepted pre0126 financial operation survives assignment and hybrid metadata expansions without historical backfill',async()=>{
...
export async function preparePreHybridReplay({ db,as,actors,financial }) {
  ...
  return async check=>{
    await check('accepted pre0128 assignment operation replays after new activity metadata without duplicate evidence',async()=>{
    ...
    await check('accepted pre0128 lifecycle start replays after new visit metadata without duplicate visit',async()=>{
    ...
    await check('accepted pre0128 financial draft replays after new activity metadata without duplicate invoice or lines',async()=>{
