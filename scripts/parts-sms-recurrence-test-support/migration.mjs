...
export async function applyPartsRecurrenceMigration(db) {
  const sql = readFileSync(new URL('../../supabase/migrations/0139_unsent_parts_sms_source_recurrence.sql', import.meta.url), 'utf8');
  for (const statement of migrationStatements(sql)) {
    try { await db.exec(statement); }
      ...
      throw new Error(`0139 ${name}: ${error.code ?? 'SQL_ERROR'} at position ${error.position ?? 'unknown'} (${error.message})`);
    }
  }
}
