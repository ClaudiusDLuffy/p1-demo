...
// This is a batch-specific ratchet against the independently verified pre-4A
// snapshot, not a replacement for the repository's ordinary TypeScript check.
const expectedChecksums = "83dbfe4b4b48ade412cb7d6586d2b8ce3ed8e085b65ab6abe321bb56d7d28689";
const repository = process.cwd();
const snapshotSetting = process.env.P1_4A_SNAPSHOT;
if (!snapshotSetting) throw new Error("P1_4A_SNAPSHOT must identify the verified pre-4A recovery snapshot");
...
assert.equal(digest(checksumsText), expectedChecksums, "Pre-4A checksum manifest does not match the verified receipt");
...
assert.ok(roots.length, "No Phase 4A roots were discovered");
...
function baselineSource(path) {
  ...
  if (entry) {
    ...
    assert.equal(digest(source), entry.sha256, `Reconstructed pre-4A source hash mismatch: ${path}`);
...
for (const path of roots) {
  ...
  for (const directive of current) {
    ...
    assert.ok(index !== -1, `New TypeScript suppression in Phase 4A root: ${path}`);
...
for (const path of legacyRoots) {
  assert.equal(digest(baselineProgram.getSourceFile(resolve(path))?.text ?? ""), digest(baselineSource(path)), `Compiler did not use pre-4A source: ${path}`);
}
