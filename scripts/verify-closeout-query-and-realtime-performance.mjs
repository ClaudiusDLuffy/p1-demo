...
const report={version:2,evidence:'PGLITE_LOCAL',generatedAt:new Date().toISOString(),
...
const outputDirectory=mkdtempSync(join(tmpdir(),'p1-phase6b-closeout-performance-'));
...
try {
  ...
  await metric('staff_table_actual','manager',"select public.list_work_orders_table_rows_v2(p_scope=>'operations',p_limit=>25,p_pending_first=>true,p_sort_column=>'created') result",[],{noTotal:true,nonempty:true});
  await metric('staff_table_count','manager',"select public.count_work_orders_table_v2(p_scope=>'operations') result");
  await metric('my_jobs_actual','contractor','select public.list_work_orders_table_rows_v2(p_scope=>\'active\',p_contractor_id=>$1,p_limit=>25,p_sort_column=>\'created\') result',[f.actors.contractor],{noTotal:true,nonempty:true});
  ...
  await metric('work_order_invoice_part_hints','manager',
    'select public.get_work_order_invoice_part_hints_v1($1,$2::uuid[]) result',
    [detailId,[syntheticId('94',2)]]);
  await metric('contractor_invoices_first','manager','select public.list_contractor_invoices_rows_v2() result',[],{noTotal:true,nonempty:true});
  await metric('contractor_invoices_count','manager','select public.count_contractor_invoices_v1() result');
  await metric('staff_invoices_first_authorized_service_scope','manager','select public.list_staff_invoices_rows_v2() result',[],{role:'service_role',noTotal:true,nonempty:true});
  ...
  await metric('navigation_visible_summary','manager','select public.get_portal_navigation_summary_v2() result');
  ...
  const roleQueries=[
    ...
    ['invoice_rows','select public.list_contractor_invoices_rows_v2() result'],
    ...
    ['navigation_count','select public.get_portal_navigation_summary_v2() result'],
  ];
  ...
  const navigationBody=(await db.query("select prosrc from pg_proc where oid='p1_portal_reads.navigation_staff_v2()'::regprocedure")).rows[0].prosrc;
  ...
  const hintPlan=await f.as('authenticated',f.actors.manager,
    tx=>captureIsolatedPlan((query,bindings)=>tx.query(query,bindings),
      'select public.get_work_order_invoice_part_hints_v1($1,$2::uuid[])',[detailId,[syntheticId('94',2)]]));
  report.plans.push({name:'work_order_invoice_part_hints',actorClass:'manager',evidence:'PGLITE_LOCAL',
    visibility:'outer production RPC; exact one-part negative-match fixture; functional positive/multi-part parity is in focused invoice harness',...summarizePlan(hintPlan)});checks++;
  phase='audit';
  ...
  assert.ok(report.measurements.find(row=>row.name==='work_order_invoice_part_hints').p95Ms<=500,
    'New parent-scoped billed-hint query meets local common-read budget');checks++;
  report.checks=checks;report.result='FINAL_SCHEMA_RETAINS_40_DISTRIBUTIONS_PLUS_PART_HINT; FOCUSED_CLOSEOUT_HARNESSES_ENFORCE_NEW_LOCAL_TIMING_GATES; HOSTED_NOT_CERTIFIED';
