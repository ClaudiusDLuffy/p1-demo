...
// This is a batch-specific ratchet against the independently verified pre-4B.1
// snapshot, not a replacement for the repository's ordinary TypeScript check.
const expectedChecksums = "27402123e7f8cbfc69ba4096e2975ff48c0dd50a4b9ddb521e6920000928f9b9";
const repository = process.cwd();
const snapshotSetting = process.env.P1_4B_CLOSEOUT_SNAPSHOT;
if (!snapshotSetting) throw new Error("P1_4B_CLOSEOUT_SNAPSHOT must identify the verified pre-4B.1 recovery snapshot");
...
assert.equal(digest(checksumsText), expectedChecksums, "Pre-4B.1 checksum manifest does not match the verified receipt");
...
assert.ok(roots.length, "No Phase 4B.1 roots were discovered");
...
function baselineSource(path) {
  ...
  let source;
  if (entry) {
    source = entry.copy === null
      ? applyZeroContextPatch(git(["show", `${context.head}:${path}`]), patch, path)
      : read(resolve(snapshot, safePath(entry.copy)));
    assert.equal(digest(source), entry.sha256, `Reconstructed pre-4B.1 source hash mismatch: ${path}`);
    if (entry.copy === null) reconstructedExcludedFiles++;
    restoredSourceFiles++;
...
for (const path of roots) {
  ...
  for (const directive of current) {
    ...
    assert.ok(index !== -1, `New TypeScript suppression in Phase 4B.1 root: ${path}`);
...
for (const path of legacyRoots) {
  assert.equal(digest(baselineProgram.getSourceFile(resolve(path))?.text ?? ""), digest(baselineSource(path)), `Compiler did not use pre-4B.1 source: ${path}`);
}
