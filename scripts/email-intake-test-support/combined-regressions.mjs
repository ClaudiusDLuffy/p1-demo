...
// the final 0130 schema. Separate older harness success alone would not prove
...
export async function verifyPriorBatchesOnFinalEmailSchema({createDatabase,applyThrough,check}) {
  ...
  try {
    ...
    const checked=(name,run)=>check(name.startsWith('REVIEW GATE')?`${name} (final 0130 compatibility only)`:`Final 0130 regression: ${name}`,run);
