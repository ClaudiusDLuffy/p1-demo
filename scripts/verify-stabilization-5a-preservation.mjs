...
const snapshot = process.env.P1_5A_SNAPSHOT;
assert(snapshot && path.isAbsolute(snapshot), "P1_5A_SNAPSHOT must identify the verified local recovery directory");
const digest = (bytes) => createHash("sha256").update(bytes).digest("hex");
const read = (relative) => readFileSync(path.join(snapshot, relative));
const manifestBytes = read("SHA256SUMS.json");
assert.equal(digest(manifestBytes), "97e2ef192f00ff96582a53be9752e1a8045ecfae02bea1637ae192de0aa57006", "Recovery manifest changed");
