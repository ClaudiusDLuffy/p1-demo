...
const apiMethodBoundary = createApiMethodBoundary("/api/private-objects/delete", ["POST"]);
...
export async function POST(request: Request) {
  const context = createRequestContext(request, "/api/private-objects/delete");
  try {
    return await runRequestOperation(context, async () => { return await finalizeApiResponse(await handlePrivateObjectRequest(request, "delete"), context); 
    });
