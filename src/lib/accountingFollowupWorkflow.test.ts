...
test("QuickBooks equipment tags are persisted atomically and validated server-side", () => {
  ...
  assert.equal(StaffInvoiceSaveSchema.safeParse(validBillingRequest()).success, true);
  assert.equal(StaffInvoiceSaveSchema.safeParse({ ...validBillingRequest(), equipmentTag: "invented" }).success, false);
});

test("quote-converted drafts have an explicit guarded ready transition", async () => {
  ...
  const response = await h.handlers.PATCH(h.request("PATCH", { action: "mark_ready" }, `?id=${financialTestIds.invoice}`));
  assert.equal(response.status, 200);
  assert.ok(h.calls.some(call => call.name === "rpc:mark_staff_invoice_ready"));
