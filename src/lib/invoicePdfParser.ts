...
function findTableColumns(header: TextRow): InvoiceTableColumns | null {
  ...
  if (
    !descriptionHeader || !amountHeader || (!qtyHeader && !rateHeader)
    || amountHeader.x <= descriptionHeader.x
  ) {
    return null;
  }
...
function extractLinesFromPage(
  ...
  for (const row of rows) {
    // Read in order: a materials header partway down a continuation page must
    // not skip service charges above it or apply its columns to those charges.
    ...
    if (
      ...
      // Close this section, not the page: another table may follow a subtotal
      // or a repeated invoice summary at the top of a continuation page.
      columns = null;
      tableEnded = true;
      continue;
    }
